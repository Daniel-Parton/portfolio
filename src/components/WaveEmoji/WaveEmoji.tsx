...
import classes from './WaveEmoji.module.css';

export interface WaveEmojiProps extends Omit<BoxProps, '__vars' | 'c' | 'h' | 'w'> {
  rippleColor?: MantineColor;
  size?: BoxProps['h'];
}
export function WaveEmoji({ className, rippleColor, size = 36, ...rest }: WaveEmojiProps) {
  let __vars: BoxProps['__vars'] | undefined;
  if (rippleColor) {
    __vars = {
      '--ripple-color': `var(--mantine-color-${rippleColor}-filled)`,
    };
  }
  return (
    <Box
